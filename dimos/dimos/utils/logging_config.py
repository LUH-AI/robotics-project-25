...
def setup_logger(*, level: int | None = None) -> Any:
    ...
    try:
        ...
        # Some environments (e.g. sandboxed runs) can create directories but still can't open
        # files outside the workspace. Fall back to a temp directory instead of crashing on import.
        fallback_dir = Path(tempfile.gettempdir()) / "dimos" / "logs"
        fallback_dir.mkdir(parents=True, exist_ok=True)
        file_handler = logging.handlers.RotatingFileHandler(
            fallback_dir / Path(log_file_path).name,
            mode="a",
            maxBytes=10 * 1024 * 1024,  # 10MiB
            backupCount=20,
            encoding="utf-8",
